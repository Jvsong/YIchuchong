...
export function generateAiCareAdvice(input: AiCareInput): AiCareAdvice {
  ...
  const intensity = input.age === "senior" || input.health !== "healthy" || input.weather !== "clear"
    ? "低到中等"
    : input.size === "large"
      ? "中高"
      : "中等";
  ...
  return {
    ...
    device: input.petType === "dog" ? "Epet 宠物定位器 + 电子围栏" : input.petType === "cat" ? "宠物监控器 + 智能饮水机" : "宠物监控器 + 安全档案",
    task: input.petType === "dog" ? "完成城市探索路线打卡" : "完成室内互动和饮水观察",
    safety: `${aiCareRules.trackerTips[input.hasTracker]} ${aiCareRules.serviceTips[input.petType]}`,
    dailyAdvice: `${input.breed}今日建议以稳定作息和可持续陪伴为主，结合日常场景安排活动。`,
    exerciseAdvice: `建议分 ${times} 次完成约 ${minutes} 分钟活动，强度控制在${intensity}。`,
    dietAdvice: input.health === "overweight" ? "控制零食和加餐，优先保持固定喂食时间。" : "保持原有主粮节奏，换粮或加餐都应循序渐进。",
